...
:::tip
No local install required — `npx -y` downloads agent-eval-kit on first run. If it's already installed locally (`pnpm add -D agent-eval-kit`), `npx` resolves to the local binary with no network lookup.
:::
...
| `mode` | `live \| replay \| judge-only` | `replay` | Execution mode |
