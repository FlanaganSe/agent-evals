...
<Tabs>
  <TabItem label="Claude Code">
    ...
    Claude Code spawns the server from the project root automatically.
  </TabItem>
...
:::tip
If agent-eval-kit is installed locally (`pnpm add -D agent-eval-kit`), `npx` resolves to the local binary with no network lookup.
:::
