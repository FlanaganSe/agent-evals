...
describe("formatMarkdownReport", () => {
	...
	it("escapes pipe characters in grader reasons", () => {
		const md = formatMarkdownReport(
			makeRun([
				makeTrial({
					status: "fail",
		...
		// Pipes in details should be escaped
		expect(md).toContain("Expected A \\| B but got C");
		// Should not break table structure
		expect(md).not.toContain("| Expected A | B");
	});

	it("escapes newlines in grader reasons", () => {
		const md = formatMarkdownReport(
			makeRun([
				makeTrial({
					status: "fail",
		...
		// Newlines should be replaced with spaces to prevent table breakage
	...
	it("contains footer with timestamp", () => {
		const md = formatMarkdownReport(makeRun([makeTrial()]));
		expect(md).toContain("Generated by agent-eval-kit");
